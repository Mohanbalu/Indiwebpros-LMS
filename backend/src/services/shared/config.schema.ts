...
export const storageConfigSchema = z.object({
  ...
  bucket: z.string().default(process.env.AWS_S3_BUCKET || process.env.AWS_BUCKET || "indiwebpros-lms-assets"),
  accessKeyId: z.string().default(process.env.AWS_ACCESS_KEY_ID || process.env.AWS_ACCESS_KEY || "mock-access-key"),
  secretAccessKey: z.string().default(process.env.AWS_SECRET_ACCESS_KEY || process.env.AWS_SECRET_KEY || "mock-secret-key"),
});
...
export const emailConfigSchema = z.object({
  ...
  smtpPassword: z.string().default(process.env.SMTP_PASSWORD || "mock-pass"),
